...
#>     mpg disp  hp drat    wt  qsec vs am carb          cars
#> 1: 21.0  160 110 3.90 2.620 16.46  0  1    4     Mazda RX4
#> 2: 21.0  160 110 3.90 2.875 17.02  0  1    4 Mazda RX4 Wag
#> 3: 22.8  108  93 3.85 2.320 18.61  1  1    1    Datsun 710
```
...
#> 5: 21.5   4 120.1  97 3.70 2.465 20.01  1  0    3    1     Toyota Corona
#> 6: 26.0   4 120.3  91 4.43 2.140 16.70  0  1    5    2     Porsche 914-2
#> 7: 15.8   8 351.0 264 4.22 3.170 14.50  0  1    5    4    Ford Pantera L
...
#>     cyl gear am  mpg disp  hp drat    wt  qsec vs carb              cars
#>  1:   4    5  1 26.0  120  91 4.43 2.140 16.70  0    2     Porsche 914-2
#>  2:   4    4  1 22.8  108  93 3.85 2.320 18.61  1    1        Datsun 710
#>  3:   4    4  0 24.4  146  62 3.69 3.190 20.00  1    2         Merc 240D
#>  4:   4    3  0 21.5  120  97 3.70 2.465 20.01  1    1     Toyota Corona
#>  5:   6    5  1 19.7  145 175 3.62 2.770 15.50  0    6      Ferrari Dino
#>  6:   6    4  1 21.0  160 110 3.90 2.620 16.46  0    4         Mazda RX4
#>  7:   6    4  0 19.2  167 123 3.92 3.440 18.30  1    4          Merc 280
#>  8:   6    3  0 21.4  258 110 3.08 3.215 19.44  1    1    Hornet 4 Drive
#>  9:   8    5  1 15.8  351 264 4.22 3.170 14.50  0    4    Ford Pantera L
#> 10:   8    3  0 18.7  360 175 3.15 3.440 17.02  0    2 Hornet Sportabout
# 或者
...
Table: (\#tab:unnamed-chunk-44)1949-1960年国际航班乘客数量变化
...
```r
# 环比横向/同比纵向
mom <- function(x) diff(x, lag = 1)/x[-length(x)]
# 格式化输出
...
```r
# base
aggregate(data = mtcars, cbind(mpg, hp) ~ cyl,
...
with(aggregate(cbind(mpg, hp) ~ cyl, mtcars, function(x) c(mean = mean(x), median = median(x))), 
     as.data.frame(cbind(cyl, mpg, hp)))
...
mtcars %>% 
  dplyr::group_by(cyl) %>% 
  dplyr::summarise(mean_mpg = mean(mpg), mean_hp = mean(hp), 
...
#> setosa.(Intercept)      0.8030518 0.34387807  2.3352806 2.375647e-02
#> setosa.Sepal.Length     0.1316317 0.06852690  1.9208760 6.069778e-02
#> versicolor.(Intercept)  0.1851155 0.51421351  0.3599974 7.204283e-01
#> versicolor.Sepal.Length 0.6864698 0.08630708  7.9538056 2.586190e-10
#> virginica.(Intercept)   0.6104680 0.41710685  1.4635770 1.498279e-01
#> virginica.Sepal.Length  0.7500808 0.06302606 11.9011203 6.297786e-16
#>                            Species
#> setosa.(Intercept)          setosa
#> setosa.Sepal.Length         setosa
#> versicolor.(Intercept)  versicolor
#> versicolor.Sepal.Length versicolor
#> virginica.(Intercept)    virginica
#> virginica.Sepal.Length   virginica
...
#> [1] magrittr_1.5      data.table_1.12.8
#> 
#> loaded via a namespace (and not attached):
#>  [1] Rcpp_1.0.5       knitr_1.29       tidyselect_1.1.0 lattice_0.20-41 
#>  [5] R6_2.4.1         rlang_0.4.6      fansi_0.4.1      highr_0.8       
#>  [9] stringr_1.4.0    dplyr_1.0.0      tools_4.0.0      grid_4.0.0      
#> [13] nlme_3.1-147     broom_0.5.6      xfun_0.15        utf8_1.1.4      
#> [17] cli_2.0.2        htmltools_0.5.0  ellipsis_0.3.1   assertthat_0.2.1
#> [21] yaml_2.2.1       digest_0.6.25    tibble_3.0.1     lifecycle_0.2.0 
#> [25] crayon_1.3.4     bookdown_0.20    tidyr_1.1.0      purrr_0.3.4     
#> [29] vctrs_0.3.1      codetools_0.2-16 curl_4.3         glue_1.4.1      
#> [33] evaluate_0.14    rmarkdown_2.3    stringi_1.4.6    compiler_4.0.0  
#> [37] pillar_1.4.4     backports_1.1.8  generics_0.0.2   pkgconfig_2.0.3
```
