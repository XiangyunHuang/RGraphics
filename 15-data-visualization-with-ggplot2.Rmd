...
[cowplot](https://github.com/wilkelab/cowplot) 是以作者 Claus O. Wilke 命名的
...
# saveRDS(maintainer_author, file = "data/maintainer_author.rds")
toc <- system.time({
  maintainer_author <- Reduce("rbind", lapply(total_maintainer, clean_net_pdb))  
}, gcFirst = TRUE)
```
```{r,echo=FALSE}
maintainer_author <- readRDS(file = "data/maintainer_author.rds")
...
```{r}
maintainer_author$weight <- 1
edges <- aggregate(weight ~ from_id + to_id, data = maintainer_author, sum)
```
