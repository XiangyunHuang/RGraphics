...
```{r,fig.cap="不同的 pch 参数值",fig.asp=1,title.mar=TRUE}
...
```{r,fig.cap="pch 支持的字符",out.width="45%",fig.width=4.5,fig.asp=1,title.mar=TRUE}
...
```{r,fig.cap="区域重叠 polygon 函数",fig.asp=1,fig.width=4.5,out.width="45%"}
...
```{r,title.mar=TRUE,fig.cap="两个 Y 轴",fig.asp=1,fig.width=5.5,out.width="55%"}
# 双Y轴
N <- 200
x <- seq(-4, 4, length = N)
y1 <- sin(x)
...
```{r category-base, fig.cap="分类散点图",fig.asp=0.8}
plot(mpg ~ hp,
...
```{r}
cumcm2011A <- readRDS(file = "data/cumcm2011A.RDS")
...
```{r unemployment,fig.cap="2009年美国各城镇失业率"}
# 代码来自 ?map
par(mar = c(0, 0, 2, 0))
library(mapproj)
data(unemp)
...
```{r,fig.cap="raster 图形",fig.asp=.8,fig.width=6,eval=require("spDataLarge")}
library(sp)
library(raster)
library(spDataLarge)
...
```{r linear-model,dev='tikz',fig.cap="线性回归模型",out.width="35%",fig.asp=1,fig.width=3.5,fig.process=to_png}
# 带有图标题
x <- rnorm(10)
y <- x + rnorm(5, sd = 0.25)
model <- lm(y ~ x)
...
```{r bessel,dev='tikz',fig.cap="贝塞尔函数",out.width="35%",fig.asp=1,fig.width=2.5,fig.process=to_png}
x0 <- 2^(-20:10)
nus <- c(0:5, 10, 20)
